...
root = Tk()
root.title("Inventory Management System for Retail")
# root.iconbitmap(r'inventory.ico')
p = ImageTk.PhotoImage(Image.open("img_1.png"))
r = Label(image=p)
r.pack(side=BOTTOM)
# canvas=Canvas(root)
...
def Database():
    ...
    cursor.execute(
        ...
        "password TEXT)")
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS `product` (product_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, product_name TEXT,"
...
def Exit():
    result = tkMessageBox.askquestion(' Inventory Management System for Retail', 'Are you sure you want to'
                                                                                 ' exit?', icon="warning")
    if result == 'yes':
        root.destroy()
        exit()
...
def ShowLoginForm():
    global loginform
    loginform = Toplevel()
    loginform.title("Inventory Management System for Retail/Account Login")
    width = 600
    height = 500
    screen_width = root.winfo_screenwidth()
    screen_height = root.winfo_screenheight()
...
def Home():
    ...
    # Title.pack(pady=10)
    lbl_display = Label(Home, text="Inventory Management System ", font=('arial', 45))
    ...
    Home.config(bg='grey')
    n = ImageTk.PhotoImage(Image.open("img_1.png"))
...
def AddNew():
    ...
    if not payment_method or amount_paid <= 0:
        messagebox.showerror("Error", "Please enter valid payment information.")
        return
    cursor.close()
    conn.close()
...
def ViewForm():
    ...
    btn_total = Button(LeftViewForm, text="Total", command=Total)
    btn_total.pack(side=TOP, padx=10, pady=10, fill=X)
...
def Delete():
    if not tree.selection():
        ...
        result = tkMessageBox.askquestion('Inventory Management System', 'Are you sure you want to delete '
...
def Total():
    # Function to calculate the total and display the total bill amount
    Database()
    cursor.execute("SELECT * FROM product")
    fetch = cursor.fetchall()
    total_bill = 0
    ...
    messagebox.showinfo("Total Bill", f"Total Bill Amount: Rs{total_bill}")

def ShowView():
    global viewform
    viewform = Toplevel()
    viewform.title("Inventory Management System for Retail/View Product")
    width = 600
    height = 400
    screen_width = Home.winfo_screenwidth()
    screen_height = Home.winfo_screenheight()
...
lbl_display = Label(root, text="Inventory Management System", font=('arial', 45), bg="Dark grey")
lbl_display.pack(fill=X)
